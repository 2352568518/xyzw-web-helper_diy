 ...
 * 后端版本：尽量与前端 `src/utils/bonProtocol.js` 的 BON 编码行为保持一致
 *
 * 注意：
 * - 这里主要是为了“编码一致性”，避免服务端出现 `-1 指令解析错误`
 * - 当前游戏服连接参数 e=x，因此实现重点为 x 加密方案 + BON 编解码
 */

// -----------------------------
// BON 编解码器核心实现（Uint8Array）
// -----------------------------
...
let _shared = new Uint8Array(524288); // 512KB
...
const x = {
  encrypt: (plainU8) => {
    const e = plainU8 instanceof Uint8Array ? plainU8 : new Uint8Array(plainU8);
  ...
  decrypt: (encryptedU8) => {
    const e = encryptedU8 instanceof Uint8Array ? encryptedU8 : new Uint8Array(encryptedU8);
    const t =
      (((e[2] >> 6) & 1) << 7) |
      (((e[2] >> 4) & 1) << 6) |
      (((e[2] >> 2) & 1) << 5) |
...
const passthrough = {
  ...
  decrypt: (e) => {
    const u8 = e instanceof Uint8Array ? e : new Uint8Array(e);
    if (u8.length > 4 && u8[0] === 112 && u8[1] === 120) return x.decrypt(u8);
    return u8;
  },
};
...
export const bon = {
  ...
  decode: (bytes) => {
    _dec.reset(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes));
    return _dec.decode();
  },
};
...
export function encodeBon(obj, encrypt = true) {
  ...
  return encrypt ? getEnc('x').encrypt(bytes) : bytes;
}
...
export function decodeBon(bytes) {
  const plain = autoDecrypt(bytes);
  return bon.decode(plain);
}

/**
 * 与前端一致的 encode：bon.encode(obj,false) → enc.encrypt → 返回 Uint8Array
 */
...
/**
 * 与前端一致的 parse：enc.decrypt → bon.decode
 */
